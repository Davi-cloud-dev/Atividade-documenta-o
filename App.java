...
public class App {
    ...
    // Configurações do localhost que vai ser criado
    static final int PORT = 8080; //Porta do localhost
    static final String CSV = "data_tasks.csv";
    static final int MAX = 5000;  //Número máximo de tarefas que podem ser criadas
    ...
    public static void main(String[] args) throws Exception {
        ...
        HttpServer server = HttpServer.create(new InetSocketAddress(PORT), 0);  //Criação do servidor HTTP
        ...
        @Override public void handle(HttpExchange ex) throws IOException {
            ...
            try {
                if ("GET".equals(method) && "/api/tasks".equals(path)) {
                    sendJson(ex, 200, listarJSON()); //vai listar todas as tarefas
                    return;
                }

                //criação de iuma nova tarefa
...
""";
    ...
    static Map<String, Object> criar(String titulo, String descr) {
        if (n >= MAX) throw new RuntimeException("Capacidade cheia"); //se o número atual de tarefas for >= ao máximo atribuído
        String id = UUID.randomUUID().toString().substring(0,8); //gera um id curto e único
        ids[n]=id; titulos[n]=titulo; descrs[n]=descr; status[n]=0; criados[n]=System.currentTimeMillis();
        n++;
        return mapOf(n-1);
    }

    //procura índice da tarefa pelo id
    ...
    //gera um JSON de todas as tarefas
